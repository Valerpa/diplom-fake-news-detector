...
async def generate_queries(req: GenerateQueriesRequest) -> QueriesResponse:
    queries = await _service.generate_queries(req.text, req.num_queries)
...
async def run_with_queries(req: RunWithQueriesRequest) -> VerifyResponse:
    result = await _service.run_with_queries(
        news_text=req.text,
        queries=req.queries,
